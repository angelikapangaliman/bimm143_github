...
> Q. Run `kmeans()`again and this time, produce 4 clusters (and call your result object `k4`) and make results figure like above?
...
**Key-point:** K-means will impose a clustering structure on your data even if it is not there - it will always give you the answer you nasked for even if that answer is silly!
...
A better way to do this is to set the rownames to the first column with `read.csv()
...
To make one of our main PCA result figures we turn to `pca$x` the scores along our new PCs. This is called "PC plot" or "Score plot" or "Orientation plot"...
