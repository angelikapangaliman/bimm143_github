...
- [Save our results to a CSV files.](#save-our-results-to-a-csv-files)
...
(hereafter “dex”), a common steroid, on airway smoooth muscle (ASM) cell
lines.
...
We will compare row by row (gene by gene) and see if the average value
in control columns is different than the average value in treated
columns.
...
a very specific way - a data strucutre setup with all the info it needs
for the calculation.
...
It is basically a plot of log2 fold change values as Adjusted P-values.

> Q. Use ggplot to make a version “volcano plot” of `log2FoldChange`
> vs. `padj`.
...
Add color to the points (genes) we care about, nice axis labels, a
...
To make sense od our results, we need to know what the differential
...
Let’s start by translating/mapping our ENSEMBLE IDs to the more
...
fold change values): and the fene sets to check overlap for.
...
```
    ...
    Info: Working in directory /Users/angelikapangaliman/Desktop/BIMM143/bimm143_github/Class13
