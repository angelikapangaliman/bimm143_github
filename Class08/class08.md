...
will save it as a separate wee vector that we can use for later to
compare our findings to those of experts.
...
optional argument `scale=TRUE` here at the data
...
The idea here is that I can take my new variables (i.e. the scores in
the PCs
`wisc.pr$x) that are better descriptors of the data set than the original features (i.e. the 30 columns ub`wisc.data\`)
and use these as a basis for clustering.
...
I can now run `table()` with both clustering `grps` and the expert
`diagnosis`
